'use strict';
/* ============================================================
 * FRIPP-Pro Playbook Process Editor
 * SVG-based interactive editor for FRIPP PlaybookProcess diagrams.
 *
 * Visual design:
...
class PBEditor {
  ...
  _renderConnections(proc) {
    ...
    for (const inst of Object.values(this.project.model.artifactStateInstances)) {
      ...
      const strokeColor = isSelected ? '#7aa3ff' : (isAchieved ? '#2ecc71' : '#4d7eff');
  ...
  _buildActivity(proc, x, y) {
    ...
    rect.setAttribute('stroke', isSelected ? '#7aa3ff' : (isCompleted ? '#2ecc71' : (hasChildren ? '#546e7a' : '#4d7eff')));
    ...
    outPort.setAttribute('stroke', '#4d7eff'); outPort.setAttribute('stroke-width', '1.5');
  ...
  _onMouseMove(e) {
    ...
    if (this.isConnecting) {
      ...
      if (fromPos) {
        ...
        line.setAttribute('stroke', '#4d7eff');
