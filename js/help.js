'use strict';
/* ============================================================
 * FRIPP-Pro Help System
 * Renders a full-screen help panel with two sections:
 *   - Tutorial: step-by-step guide to using FRIPP-Pro
    ...
    ov.innerHTML = `
      <div class="help-panel">
        <div class="help-header">
          <span class="help-title">FRIPP-Pro Help</span>
          <div class="help-tabs">
...
<p>
  <strong>FRIPP-Pro</strong> is a browser-based modelling tool for
  <em>Operations-Informed Incident Response Playbook Planning</em>.
  It implements the FRIPP metamodel (Shaked et al. 2023) which combines:
</p>
<ul>
...
<div class="tut-note">
  All models created in FRIPP-Pro strictly conform to the FRIPP metamodel.
...
<div class="tut-steps">
  <div class="tut-step">
    ...
    <div class="tut-step-body">
      ...
      <code>examples/</code> folder. Start with <code>examples/example.json</code> —
      a complete Web Injection Attack Response scenario with a Dependency Model and Playbook already connected.
    </div>
  </div>
  <div class="tut-step">
...
<ol>
  ...
  <li>Drag from the <span style="color:#4d7eff">blue circle</span> on an activity's right edge
...
<h3>Auto-Save</h3>
<p>
  FRIPP-Pro automatically saves the project to <code>localStorage</code> (debounced, 1.5 s after
  each change). On next page load, the last session is automatically restored.
</p>

<h3>Save / Open</h3>
<p>
  <kbd>Save</kbd> downloads a <code>.fripp-pro.json</code> file containing both the
...
<ul>
  ...
  <li><code>.fripp</code> — SecMoF XML playbook</li>
  <li><code>.json</code> — FRIPP-Pro model-only JSON export</li>
</ul>
...
<table style="width:100%;border-collapse:collapse;font-size:12px">
  ...
  <tbody>
    <tr><td style="padding:5px 10px;border-bottom:1px solid var(--border)"><code>example.json</code></td>
        <td style="padding:5px 10px;border-bottom:1px solid var(--border)"><strong>Web Injection Attack Response</strong> — the primary FRIPP publication scenario (Shaked et al. 2023). Includes DM, playbook, ActivityImpact connections, and CiO analysis.</td></tr>
    <tr><td style="padding:5px 10px;border-bottom:1px solid var(--border)"><code>example-hazmat.json</code></td>
        <td style="padding:5px 10px;border-bottom:1px solid var(--border)"><strong>Hazardous Materials Spill</strong> — industrial incident response scenario.</td></tr>
    <tr><td style="padding:5px 10px;border-bottom:1px solid var(--border)"><code>example-flood.json</code></td>
        <td style="padding:5px 10px;border-bottom:1px solid var(--border)"><strong>Flood Response</strong> — multi-stakeholder emergency response playbook.</td></tr>
    <tr><td style="padding:5px 10px;border-bottom:1px solid var(--border)"><code>example-pr-crisis.json</code></td>
        <td style="padding:5px 10px;border-bottom:1px solid var(--border)"><strong>PR Crisis Response</strong> — communications-focused incident response.</td></tr>
    <tr><td style="padding:5px 10px;border-bottom:1px solid var(--border)"><code>example-loto.json</code></td>
        <td style="padding:5px 10px;border-bottom:1px solid var(--border)"><strong>Lock-Out/Tag-Out Procedure</strong> — operational safety playbook.</td></tr>
    <tr><td style="padding:5px 10px;border-bottom:1px solid var(--border)"><code>GlobalShippingWithBaselines.json</code></td>
        <td style="padding:5px 10px;border-bottom:1px solid var(--border)"><strong>Global Shipping Disruption</strong> — multi-DM supply chain scenario with pre-captured snapshots demonstrating baseline and recovery states.</td></tr>
    <tr><td style="padding:5px 10px;border-bottom:1px solid var(--border)"><code>BillingServiceWithBaselines.json</code></td>
        <td style="padding:5px 10px;border-bottom:1px solid var(--border)"><strong>Billing Service Outage</strong> — financial services incident scenario with pre-captured snapshots.</td></tr>
  </tbody>
</table>

<p style="margin-top:12px">
  The <code>example.json</code> scenario demonstrates:
</p>
<ul>
  <li><strong>CiO analysis</strong>: the Database Availability paragon has the highest CiO, so restoring the database has the largest positive operational impact.</li>
  <li><strong>ActivityImpacts</strong>: Remediation activities (block IP, patch vulnerability, restore DB) are linked to DM paragons.</li>
...
`;
  ...
  _renderMetamodels(container) {
    ...
    FRIPP_METAMODEL.packages.forEach((pkg, i) => {
    ...
    footer.innerHTML =
      `<strong>${FRIPP_METAMODEL.name}</strong> &nbsp;v${FRIPP_METAMODEL.version} &nbsp;&mdash;&nbsp; ` +
      FRIPP_METAMODEL.description;
...
<div class="about-wrap">
  <div class="about-logo">FRIPP-Pro</div>
  <div class="about-subtitle">Operations-Informed Incident Response Playbook Planner</div>

  <div class="about-section">
    <h3>What is FRIPP-Pro?</h3>
    <p>
      FRIPP-Pro is a browser-based modelling tool implementing the FRIPP metamodel for
  ...
  <div class="about-section">
    ...
    <p>${FRIPP_METAMODEL.name} &mdash; v${FRIPP_METAMODEL.version}</p>
  ...
  <div class="about-section">
    ...
    <p>
      FRIPP-Pro is inspired by <strong>SecMoF</strong>, an Eclipse-based reference implementation
