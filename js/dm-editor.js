'use strict';
/* ============================================================
 * FRIPP-Pro Dependency Model Editor
...
class DMEditor {
  ...
  _buildNode(paragon, x, y) {
    ...
    else if (isProxy)      borderColor = isSelected ? '#7aa3ff' : '#4a7a99';
    else if (isSelected)   borderColor = '#7aa3ff';
    ...
    if (!isProxy) {
      ...
      port.setAttribute('fill', '#1e2d45'); port.setAttribute('stroke', '#4d7eff');
  ...
  _bindEvents() {
    ...
    this.svg.addEventListener('drop', e => {
      ...
      const raw = e.dataTransfer.getData('application/fripp-paragon');
