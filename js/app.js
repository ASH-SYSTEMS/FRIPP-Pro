'use strict';
/* ============================================================
 * FRIPP-Pro Main Application Controller
...
class FRIPPApp {
  ...
  init() {
    ...
    const saved = localStorage.getItem('fripp-pro-autosave');
    if (saved) {
      try {
  ...
  _cacheEls() {
    const ids = [
      ...
      'dm-view','pb-view','metrics-view','symbiosis-view','impact-view',
  ...
  _renderParagonTree(id, paragons, container, dmId, depth = 0) {
    ...
    item.addEventListener('dragstart', e => {
      e.dataTransfer.setData('application/fripp-paragon', JSON.stringify({ dmId, paragonId: id }));
  ...
  _initPanelResize() {
    const MIN_W = 80;
    const storage = { left: 'fripp-panel-left', right: 'fripp-panel-right' };
  ...
  _renderParagonCT(paragon, dm) {
    ...
    for (const stk of stakeholders) {
      ...
      const roleBadge   = linkedRole ? ` <span style="font-size:9px;color:#4d7eff;background:#0d1e3a;padding:1px 4px;border-radius:3px;margin-left:4px;">Role</span>` : '';
      const li = document.createElement('li');
  ...
  _renderMetrics() {
    ...
    for (const { dm, rootProbability, paragons } of report.dmReports) {
      ...
      stats.appendChild(this._statCard(paragons.length, 'Paragons', '#4d7eff'));
  ...
  _newProject() {
    ...
    localStorage.removeItem('fripp-pro-autosave');
    this.project = createProject('New Project');
  ...
  async _onFileSelected(e) {
    ...
    try {
      ...
      if (ext === 'dependencymodel') {
        ...
        const result = Storage.importFRIPPXML(text);
        this._pushUndo();
        ...
        this.project = proj;
        this._currentFilename = file.name.replace(/\.fripp-pro\.json$/i, '').replace(/\.json$/i, '');
        this._loadProject();
        this._updateFilenameDisplay();
  ...
  _saveProject() {
    ...
    const base = this._currentFilename || (this.project.name || 'fripp-project').replace(/\s+/g, '-').toLowerCase();
    Storage.downloadJSON(json, base + '.fripp-pro.json');
  ...
  _importFile() {
    ...
    input.type = 'file';
    input.accept = '.json,.dependencymodel,.fripp';
  ...
  _exportModelOnly() {
    ...
    const name = (this.project.name || 'fripp-model').replace(/\s+/g,'-').toLowerCase();
  ...
  async _openLibrary() {
    ...
    for (const item of manifest) {
      html += `<div class="library-item" data-file="${this._esc(item.filename)}"
        ...
        <div style="font-size:10px;color:#4d7eff;margin-top:3px">${this._esc(item.filename)}</div>
      </div>`;
    }
    html += '</div>';
    ...
    this.els['modal-body'].querySelectorAll('.library-item').forEach(item => {
      item.addEventListener('mouseenter', () => { item.style.borderColor = '#4d7eff'; });
  ...
  async _loadExampleFile(filename) {
    ...
    try {
      ...
      this._currentFilename = filename.replace(/\.fripp-pro\.json$/i, '').replace(/\.json$/i, '');
      this._updateFilenameDisplay();
      this._loadProject();
  ...
  _scheduleAutoSave() {
    ...
    this._autoSaveTimer = setTimeout(() => {
      try {
        localStorage.setItem('fripp-pro-autosave', Storage.serialize(this.project));
      } catch (e) {
