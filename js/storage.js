 ...
 * Also supports importing model-only files (.dependencymodel, .fripp)
 * ============================================================ */
...
const Storage = {
  ...
  /* ----------------------------------------------------------
   * Import a SecMoF .fripp XML file (simplified import)
   * Creates a PlaybookProcess tree from the XMI
   * ---------------------------------------------------------- */
  importFRIPPXML(xmlString) {
    const parser = new DOMParser();
    const doc = parser.parseFromString(xmlString, 'application/xml');

    if (doc.querySelector('parsererror')) {
      throw new Error('Invalid XML in FRIPP file');
    }
