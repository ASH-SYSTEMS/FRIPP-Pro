...
class ImpactView {
    ...
    const collect = (procId, depth) => {
      ...
      if (depth > 0) {
        ...
        if (hasKids) {
          ...
          this._attrs(tri, {
            x: x + 9, y: y + AH / 2 + 4,
            fill: '#4d7eff', 'font-size': 10, 'text-anchor': 'middle',
            'font-weight': 'bold'
          });
          ...
          if (isCollapsed) {
            ...
            this._attrs(badge, {
              x: x + w - 22, y: y + AH / 2 + 4,
              fill: '#4d7eff', 'font-size': 9, 'text-anchor': 'middle'
            });
        ...
        if (this.connectMode) {
          ...
          this._attrs(h, { cx: x + w, cy: y + AH / 2, r: 6,
                           fill: '#4d7eff', stroke: '#fff', 'stroke-width': 1.5 });
  ...
  _renderDM(vp, paneX, paneY, paneW, paneH) {
    ...
    for (const id of reachable) {
      ...
      const stroke = isActive ? '#4d7eff' : ((isImp || changed) ? '#ff7043' : Metrics.probToColor(prob));
      ...
      if (isProxy) {
        ...
        this._attrs(badge, { x: pos.x + nW - 3, y: pos.y + bfs + 2, 'text-anchor': 'end',
                             fill: '#7aa3ff', 'font-size': bfs, 'pointer-events': 'none' });
        badge.textContent = `\u2197${this._trunc(proxyDMName, 10)}`;
        g.appendChild(badge);
      } else {
        const typeColors = { AND: '#4d7eff', OR: '#9b59b6', UNCONTROLLABLE: '#e67e22' };
        const badge = this._el('text');
        this._attrs(badge, { x: pos.x + 3, y: pos.y + bfs + 2,
                             fill: typeColors[p.type] || '#4d7eff', 'font-size': bfs,
                             'pointer-events': 'none' });
      ...
      if (!isProxy) {
        ...
        this._attrs(port, { cx: pos.x + nW, cy: pos.y + nH / 2, r: Math.max(4, 5 * Z),
                            fill: '#1e3a5f', stroke: '#4d7eff', 'stroke-width': 1 });
    ...
    if (this._dmConnectFrom) {
      ...
      if (fromN) {
        ...
        if (!dl) {
          ...
          this._attrs(dl, { fill: 'none', stroke: '#4d7eff',
  ...
  _panelLabel(vp, x, y, text) {
    const t = this._el('text');
    this._attrs(t, { x, y, fill: '#4d7eff', 'font-size': 10,
